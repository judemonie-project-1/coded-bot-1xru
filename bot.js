...
loadState();
var _IMG1=path.join(__dirname,'coded.jpg');
...
var NOT_LIVE=['$CODED hasn\u2019t launched yet. CA coming soon.','Not yet. Stay ready.','CA drops soon. Hold tight.'];
var CTO_REPLIES=['$CODED is a CTO. Original dev gone. Community owns and runs this completely. No dev to rug.','CTO project. Dev walked away. Community stepped up and owns $CODED now. That is the strength.','No dev here. $CODED is 100% community-owned. Original dev left. Community drives this forward.'];
function sysPrompt(){
  return 'You are the community bot for $CODED, a BNB Smart Chain (BSC) meme token.\nToken: $CODED | Chain: BNB Smart Chain (BSC)\nSupply: 1,000,254,582\nTax: 0% buy / 0% sell\nContract: NOT RENOUNCED | LP: BURNED\nDEV: CTO. Original dev gone. Community owns $CODED completely. Say this clearly when asked.'+(TWITTER?'\nTwitter: '+TWITTER:'')+'\nNarrative: '+"$CODED CTO is a community-driven meme coin on the BNB Smart Chain built for believers who know the next big move is already coded. Powered by pure community takeover energy, strong meme culture, and relentless holders, $CODED represents conviction, bullish"+'\nPersonality: Warm, inclusive, friendly. Make everyone feel welcome. Genuine and supportive.\nRULES: 2-4 lines max. Natural and professional. Never share TG group link. Never repeat reply. If hype/casual/no question: reply IGNORE exactly.';
}
...
var SIL_ANG=['2-3 lines. Why hold $CODED right now.','2-3 lines. $CODED fundamentals: renounced, LP locked.','2-3 lines. Being early to $CODED.','2-3 lines. $CODED community is building.','2-3 lines. The move in $CODED is still early.'];
var silIdx=0;
...
async function doShoutout(){
  ...
  try{
    ...
    var ppt='Write 1-2 warm genuine lines appreciating these $CODED admins for keeping the community alive: '+names.join(', ')+'. Be specific, sound human, tag them by name.';
...
bot.command('ca',async function(ctx){if(!caUnlocked)return ctx.reply(NOT_LIVE[Math.floor(Math.random()*NOT_LIVE.length)]);await sendWithTracker(caMsg,ctx.chat.id,'$CODED Contract Address',{});return ctx.reply('<code>'+CA+'</code>',{parse_mode:'HTML'});});
bot.command('x',async function(ctx){return sendWithTracker(xMsg,ctx.chat.id,'Follow $CODED on X',{reply_markup:{inline_keyboard:[[{text:'Follow on X',url:TWITTER}]]}});});
bot.command('twitter',async function(ctx){return sendWithTracker(xMsg,ctx.chat.id,'Follow $CODED on X',{reply_markup:{inline_keyboard:[[{text:'Follow on X',url:TWITTER}]]}});});
bot.command('socials',function(ctx){return ctx.reply('<a href=\'https://dexscreener.com/bsc/0xeFA54451b73B5eB8701b8Df4feDEfD615c274444\'>Chart</a> | <a href=\'https://pancakeswap.finance/swap?outputCurrency=0xeFA54451b73B5eB8701b8Df4feDEfD615c274444\'>PancakeSwap</a>'+(TWITTER?' | <a href=\''+TWITTER+'\'>Twitter</a>':'')+(WEBSITE?' | <a href=\''+WEBSITE+'\'>Website</a>':''),{parse_mode:'HTML',disable_web_page_preview:true});});
bot.command('links',function(ctx){return ctx.reply('<a href=\'https://dexscreener.com/bsc/0xeFA54451b73B5eB8701b8Df4feDEfD615c274444\'>Chart</a> | <a href=\'https://pancakeswap.finance/swap?outputCurrency=0xeFA54451b73B5eB8701b8Df4feDEfD615c274444\'>PancakeSwap</a>'+(TWITTER?' | <a href=\''+TWITTER+'\'>Twitter</a>':'')+(WEBSITE?' | <a href=\''+WEBSITE+'\'>Website</a>':''),{parse_mode:'HTML',disable_web_page_preview:true});});
bot.command('info',function(ctx){return ctx.reply('<b>$CODED</b> \u2014 BNB Smart Chain (BSC)\n\nSupply: 1,000,254,582\nTax: 0% buy / 0% sell\nContract: NOT RENOUNCED\nLP: BURNED'+(TWITTER?'\nTwitter: '+TWITTER:''),{parse_mode:'HTML',disable_web_page_preview:true});});
bot.command('shill',async function(ctx){
  var shillMsgs=[
    'Have you heard about $CODED?\n\n$CODED \u2014 community-owned on BSC.\nRenounced. LP BURNED. No dev games.\nThis is the quiet move. Load up.',
    'Looking for a BSC token built by real people?\n\n$CODED \u2014 fully community-owned.\nRenounced contract. LP BURNED. Real narrative.\nGet in early \u261d',
    'The move others will regret missing.\n\n$CODED on BSC \u2014 community takeover.\nRenounced. LP BURNED. No rug possible.\nLoad up before it runs.',
    'What if the next gem was right here?\n\n$CODED \u2014 zero dev, 100% community.\nRenounced. LP BURNED. Low cap. Real conviction.',
    'Don\u2019t sleep on $CODED.\nCommunity took over. Dev is gone. LP BURNED.\nThis is what conviction looks like. Load up.',
  ];
...
bot.on('message',async function(ctx){
  ...
  if(admin&&!isPrivate){
    ...
    if(caW.some(function(w){return lower===w||lower.includes(w);})){
      ...
      await sendWithTracker(caMsg,ctx.chat.id,'$CODED Contract Address',{});return ctx.reply('<code>'+CA+'</code>',{parse_mode:'HTML'});
    }
    if(lower==='x'||lower==='twitter')return sendWithTracker(xMsg,ctx.chat.id,'Follow $CODED on X',{reply_markup:{inline_keyboard:[[{text:'Follow on X',url:TWITTER}]]}});
    if(lower==='socials'||lower==='links')return ctx.reply('<a href=\'https://dexscreener.com/bsc/0xeFA54451b73B5eB8701b8Df4feDEfD615c274444\'> Chart</a> | <a href=\'https://pancakeswap.finance/swap?outputCurrency=0xeFA54451b73B5eB8701b8Df4feDEfD615c274444\'> PancakeSwap</a>'+(TWITTER?' | <a href=\''+TWITTER+'\'>Twitter</a>':''),{parse_mode:'HTML',disable_web_page_preview:true});
    return;
  }
  if(!text)return;
  ...
  if(lower2==='x'||lower2==='twitter'||lower2.includes('follow on'))return sendWithTracker(xMsg,ctx.chat.id,'Follow $CODED on X',{reply_markup:{inline_keyboard:[[{text:'Follow on X',url:TWITTER}]]}});
  if(lower2==='socials'||lower2==='links')return ctx.reply('<a href=\'https://dexscreener.com/bsc/0xeFA54451b73B5eB8701b8Df4feDEfD615c274444\'> Chart</a> | <a href=\'https://pancakeswap.finance/swap?outputCurrency=0xeFA54451b73B5eB8701b8Df4feDEfD615c274444\'> PancakeSwap</a>'+(TWITTER?' | <a href=\''+TWITTER+'\'>Twitter</a>':''),{parse_mode:'HTML',disable_web_page_preview:true});
...
app.listen(PORT,async function(){console.log('$CODED bot port '+PORT);try{await new Promise(function(r){setTimeout(r,2000);});}catch(_){}try{await regWH();}catch(e){console.log(e.message);}if(parseInt(SIL_DELAY||'0')>0)try{resetSil();}catch(_){}try{schedShout();}catch(_){}setInterval(function(){if(WEBHOOK_URL)try{fetch(WEBHOOK_URL+'/health').catch(function(){});}catch(_){}},4*60*1000);console.log('$CODED bot live');});